...
- **Opposition considered:** "1.96% is embarrassingly low; why show it?" Resolution: showing a historical baseline, heavily caveated, is more credible than hiding it or replacing it with a flattering current number. The deck's integrity depends on the audience trusting the one number it does show.
...
- **Speaker notes embedded in `build.mjs` via `addNotes`** — verified they survive into the produced PPTX (notes slide XML present for all 5 pages).
