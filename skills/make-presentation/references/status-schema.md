...
node .agents/skills/make-presentation/scripts/status.mjs set-engine ./work/demo open-slide
node .agents/skills/make-presentation/scripts/status.mjs result ./work/demo production-qa passed "Exports verified" --artifact QA-REPORT.md --artifact dist/demo.pptx --artifact dist/demo.pdf --input slides/demo-deck/index.tsx --decision "Approve exports for the chosen engine"
...
`init` requires `<root>`, `<deck-id>`, and `<source-root>`; `<mode>` and `<mode-reason>` are optional and default to `general`. `--force` overwrites an existing `STATUS.md`. Deck ids must be safe kebab-case. `set-engine <root> <engine>` records the slide engine chosen at the deck-prototype gate so the deck-qa gate inherits it; `engine` is `null` until then. A passed `result` requires at least one `--artifact`; repeat it for multiple outputs. Repeat `--input` and `--decision` to replace those arrays with the values supplied in that command. `next` prints the earliest gate whose status is not `passed`, or `complete`. Artifact and input paths are opaque strings to the helper — the example values above (`.pptx`, `.pdf`, `index.tsx`) illustrate one engine's outputs; whatever the chosen engine produces goes in the same slots.
