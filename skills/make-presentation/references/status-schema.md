...
`init` refuses to overwrite an existing `STATUS.md`; `--force` is an explicit reset. Deck ids must be safe kebab-case. A passed `result` requires at least one `--artifact`; repeat it for multiple outputs. Repeat `--input` and `--decision` to replace those arrays with the values supplied in that command. `next` prints the earliest gate whose status is not `passed`, or `complete`. Artifact and input paths are opaque strings to the helper — the example values above (`.pptx`, `.pdf`, `index.tsx`) illustrate one engine's outputs; whatever the chosen engine produces goes in the same slots.
